...
local function buildAirtightLookupTable(...)
    local dirs = {...}
    local lookup = {}
    for _, dir in ipairs(dirs) do
        for tile_name in pairs(ZSpaceship.MapData.Tiles[dir]) do
...
ZSpaceship.MapData.AIRTIGHT_WALLS_NS_LOOKUP = buildAirtightLookupTable("WallN", "WallSE")

-- East/West walls (getWall(false)) - from WallW flag, only if also Airtight
ZSpaceship.MapData.AIRTIGHT_WALLS_EW_LOOKUP = buildAirtightLookupTable("WallW", "WallSE")

-- Door tiles (North/South direction) - from DoorN flag, only if also Airtight
ZSpaceship.MapData.AIRTIGHT_DOORS_NS_LOOKUP = buildAirtightLookupTable("DoorN")

-- Door tiles (East/West direction) - from DoorW flag, only if also Airtight
ZSpaceship.MapData.AIRTIGHT_DOORS_EW_LOOKUP = buildAirtightLookupTable("DoorW")
