...
-- Initialize default rooms from MapData.DefaultRooms (called after MapData is loaded)
local function initDefaultRooms()
    if DEFAULT_ROOMS == nil and ZSpaceship.MapData and ZSpaceship.MapData.DefaultRooms then
        DEFAULT_ROOMS = {}
        for i, room in ipairs(ZSpaceship.MapData.DefaultRooms) do
            DEFAULT_ROOMS[i] = {
                x = room.x,
                y = room.y,
                z = room.z,
...
local function loadRoomData(isNewGame)
    ...
    -- Initialize defaults from DefaultRooms
    local hasDefaults = initDefaultRooms()
    
    if isNewGame then
        -- New game: fill MapData.Rooms from DefaultRooms
        ...
        elseif ZSpaceship.MapData.DefaultRooms then
            -- Fallback: use DefaultRooms directly if DEFAULT_ROOMS not initialized
            fillMapDataRooms(ZSpaceship.MapData.DefaultRooms)
            print("ZSpaceship: Initialized room data for new game from DefaultRooms (" .. #ZSpaceship.MapData.Rooms .. " rooms)")
        end
        return
    end
    ...
    elseif ZSpaceship.MapData.DefaultRooms then
        -- Fallback: use DefaultRooms directly
        fillMapDataRooms(ZSpaceship.MapData.DefaultRooms)
        print("ZSpaceship: Using DefaultRooms (no saved data found) (" .. #ZSpaceship.MapData.Rooms .. " rooms)")
    end
end
    ...
    Events.OnGameStart.Add(function()
        ...
        if not ZSpaceship.MapData.Rooms or #ZSpaceship.MapData.Rooms == 0 then
            ...
            if not hasSavedData then
                ...
                elseif ZSpaceship.MapData.DefaultRooms then
                    fillMapDataRooms(ZSpaceship.MapData.DefaultRooms)
                    print("ZSpaceship: Initialized room data from DefaultRooms on GameStart (" .. #ZSpaceship.MapData.Rooms .. " rooms)")
                end
            end
        end
    end)
