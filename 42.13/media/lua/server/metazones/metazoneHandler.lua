-- Override of vanilla metazoneHandler.lua (B42) to harden RoomTone loading.
-- Vanilla can crash with:
--   NPE: IsoMetaGrid.registerRoomTone -> cell.roomTones (cell is nil)
-- when a roomtones.lua entry references a cell outside the current metagrid bounds.
--
-- We keep vanilla behavior but wrap registerRoomTone in pcall so the game doesn't crash.
...
local function handleRoomTone(file, v)
	...
	-- Avoid Java NPE: IsoMetaGrid.registerRoomTone assumes getCellData(x/256,y/256) is non-nil.
	local metaGrid = getWorld() and getWorld():getMetaGrid()
	if metaGrid then
		local cell = metaGrid:getCellData(math.floor(v.x / 256), math.floor(v.y / 256))
		if not cell then
			print('WARN: Skipping out-of-range RoomTone in '..file..' at '..tostring(v.x)..','..tostring(v.y)..','..tostring(v.z))
			return
		end
	end
	...
	if not ok then
		print('WARN: Skipping invalid RoomTone in '..file..' at '..tostring(v.x)..','..tostring(v.y)..','..tostring(v.z)..' : '..tostring(err))
	end
end
			...
			for k,v in ipairs(objects) do
				...
				if v.type ~= "Vegitation" and v.type ~= "DeepForest" and v.type ~= "Forest" and v.type ~= "TownZone" and v.type ~= "Farm" and v.type ~= "FarmLand" and v.type ~= "TrailerPark" then
					...
					elseif v.type == "RoomTone" then
						-- handled by roomtones.lua (see loadRoomTones)
