...
local function registerDoorUnlocks()
    ...
    for sprite in pairs(ZSpaceship.MapData.DoorSprites) do
        ...
        MapObjects.OnLoadWithSprite(sprite, unlockDoor, 10) -- map loading?
    end
end
...
-- OnGameStart runs on every game _LOAD_, i.e. if smth wasnt fixed because player initially spawned outside of the Space, then game save-load should fix it
-- but OnGameStart is too late for registerDoorUnlocks() because map is already fully loaded, so OnNewObjectWithSprite are already fired
